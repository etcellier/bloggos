<x-app-layout>
    <x-slot name="header">
        <h2 class="flex justify-between items-center font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Categories') }}
            <a href="{{ route('category.add') }}" class="rounded-full bg-indigo-400 text-white p-1 shadow-sm hover:shadow hover:bg-indigo-200 hover:text-indigo-400 transition-all dark:bg-indigo-600 dark:hover:bg-indigo-400 dark:hover:text-indigo-600">
                    ...
                    <a href="{{ route("category.update", [$id = $category->id]) }}">
                        <div class="p-6 col-4 text-gray-900 dark:text-gray-100 bg-white rounded hover:shadow transition-all flex justify-between h-full gap-2">
                            <div class="flex justify-start gap-2 items-center">
                                ...
                                {{ $category->name }}
                                <span class="text-slate-400 font-light text-base">(/{{ $category->slug }})</span>
                            </div>
